...
async def call_gemini_text_json(system: str, prompt: str, fallback: Optional[dict] = None) -> dict:
    last_text = ""
    for attempt in range(3):
        ...
        if parsed is not None:
            return parsed
        logger.warning("Gemini text returned unparseable JSON (attempt %s): %r", attempt + 1, last_text[:200])
