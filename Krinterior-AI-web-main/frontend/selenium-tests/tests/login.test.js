...
const loginPath = process.env.LOGIN_PATH || "/Krinterior-AI/web/#/login";
...
describe("Krinterior AI web login", function () {
  this.timeout(60000);
  let driver;
  ...
  it("submits valid credentials when E2E secrets are configured", async function () {
    ...
    await driver.wait(async () => {
      ...
      return currentUrl.includes("#/dashboard") || currentUrl.includes("/dashboard");
    }, 20000);

    const currentUrl = await driver.getCurrentUrl();
    assert.match(currentUrl, /dashboard/);
  });
});
